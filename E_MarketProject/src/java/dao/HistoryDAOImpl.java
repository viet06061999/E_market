...
public class HistoryDAOImpl implements HistoryDAO {
	...
	public void addHistory(History h) {
		...
		String sql = "insert into history values ("+h.getUser_id()+","+h.getMa_san_pham()+",'"+h.getNgay_mua()+"',"+h.getSo_luong()+","+h.getThanh_tien()+")";
	...
	public List<History> getList(int id) {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new History( user_id, ma_san_pham, ngay_mua, so_luong, thanh_tien));
			}
			con.close();
		} catch (SQLException e) {
			e.printStackTrace();
